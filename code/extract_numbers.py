...
extract_numbers.py - 3rd script in sequence 1.) download_images.py, 2.) convert_gif_to_bmp.py, 3.) extract_numbers.py, 4.) pivot_and_join.py
                     ...
                     Specifically, extract daily min/max temperature prediction (Fahrenheit),
                       and probability of precipitation prediction (out of 100%) from archived/historical image files
...
Developed by Bruce Aker   12/5/15 - 1//16
'''
...
with open('pixel_location_of_w_nums.py', mode='rU') as f:
...
'''
data_dir_from = '../data/converted_data/' #BMP files
...
#create or overwrite file with header (column titles)
...
for issue_day in range(1, max_day_in_month[issue_month-1]+1):
  ...
  for forecast_day in ['3','4','5','6','7']: #forecast is valid for a date that is this many days ahead
    ...
    for forecast_type in ['MIN','MAX','POP1','POP2']: #min/max temp prediction, probability of precipitation prediction for 12Z/00Z, Z=UTC
      file_name = issue_date + '12_DAY' + forecast_day + '_' + forecast_type + '_filled.bmp'
      black_idx = -1 #initially invalid value

      print 'For file spec ' + data_dir_from + file_name
      ...
      for wea_stn in chars_origins:
        sys.stdout.write('For weather station code %s ' % wea_stn['icao_code']) # char's location in image corresponds to a weather station location
        ...
        for i_x_offset in range(5): #Cycle thru number of char's (and +/-) which affects the x-offset,
          ...
          for i_row in range(13): #cycle thru pixel rows (of all 3 positions) within the characters in image
          ...
          if found_num(found_char, x_offset_for_num_chars[i_x_offset]['num_chars'],x_offset_for_num_chars[i_x_offset]['sign']):
            ...
            print 'found weather number', weather_number
            break

        if weather_number == invalid_wea_num:
          print 'did not find a weather number'

        with open(data_file_to,mode='a') as f:
          f.write('\n%s,%s,%s,' % (wea_stn['icao_code'],issue_date,forecast_type.lower() + '_' + forecast_day.lower()))
          if weather_number != invalid_wea_num:
            f.write('%i,%u%02u%02u' % (weather_number,valid_date_calcd.year,valid_date_calcd.month,valid_date_calcd.day))
          else:
            f.write(',%u%02u%02u' % (valid_date_calcd.year,valid_date_calcd.month,valid_date_calcd.day))
